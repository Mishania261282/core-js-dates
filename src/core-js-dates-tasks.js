...
function getNextFriday(date) {
  ...
  nextFriday.setDate(nextFriday.getUTCDate() + days[nextFriday.getUTCDay()]);
  return nextFriday;
}
...
function isDateInPeriod(date, period) {
  ...
  return dateNow - startDate >= 0 && endDate - dateNow >= 0;
}
...
function formatDate(date) {
  ...
  const MD = hours < 12 ? 'AM' : 'PM';
  return `${month}/${day}/${year}, ${hours > 12 ? hours - 12 : hours}:${minutes}:${seconds} ${MD}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const numberOfDays = Math.floor(date - beginDate) / (1000 * 60 * 60 * 24);
  const weekNumber = Math.ceil(numberOfDays / 7);
  return beginDate.getDay() === date.getDay() ? weekNumber + 1 : weekNumber;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const [endDay, endMonth, endYear] = period.end
    .split('-')
    .map((item) => Number(item));
  const startDate = new Date(startYear, startMonth - 1, startDay, 3);
  const endDate = new Date(endYear, endMonth - 1, endDay, 3);
